...
const unsigned char ihl = 5;

void packeting(char* Packet,unsigned char* version_IHL,
               unsigned char* differentiatedServices,unsigned short* totalLength,
               short* identification,unsigned short* x_DF_MF_FragmentOffset,
               unsigned char* timeToLive, unsigned char* protocol,
               short* headerChecksum,unsigned int* sourceAddress,
               unsigned int* destinationAddress,char *datagram,unsigned int size){
    memcpy(Packet, version_IHL, 1);
    memcpy(Packet+1,differentiatedServices,1);
    memcpy(Packet+2,totalLength,2);
    memcpy(Packet+4,identification,2);
    memcpy(Packet+6,x_DF_MF_FragmentOffset,2);
    memcpy(Packet+8,timeToLive,1);
    memcpy(Packet+9,protocol,1);
    memcpy(Packet+10,headerChecksum,2);
    memcpy(Packet+12,sourceAddress,4);
    memcpy(Packet+16,destinationAddress,4);
    memcpy(Packet+20,datagram,size);
}

void connectVersionAndIHL(unsigned char* VersionAndIHL,const unsigned char* version,const unsigned char* IHL){
    *VersionAndIHL= *version << 4;
    *VersionAndIHL += *IHL;
}

void generateDifferentiatedServices(unsigned char* ds,unsigned char priority,unsigned char delay,
                                    unsigned char throughput,unsigned char reliability){
    priority = priority << 5;
    delay = delay << 4;
    throughput = throughput << 3;
    reliability = reliability << 2;
    *ds = priority + delay + throughput + reliability;
}
...
void calculateChecksum(char * packet){
    ...
    if (headerChecksum>0xFFFF){
        overflow = (short)(headerChecksum / 0xFFFF);
        headerChecksum += overflow;
    }
    shortChecksum = (short)~headerChecksum;
    memcpy(packet+10,&shortChecksum,2);
}
...
// source mac address
unsigned char src_mac_address[6]={0x12,0x34,0x56,0x78,0x9a,0xbc};

// destination mac address
unsigned char dst_mac_address[6]={0x13,0x34,0x56,0x78,0x9a,0xbc};
...
short setDF(int DF, unsigned short X_DF_MF_FragmentOffset){
    if (DF){
        return X_DF_MF_FragmentOffset | 0x4000;
    }
    else{
        return X_DF_MF_FragmentOffset & 0xBFFF;
    }
}
short setMF(int MF, short X_DF_MF_FragmentOffset){
    if (MF){
        return X_DF_MF_FragmentOffset | 0x2000;
    }
    else{
        return X_DF_MF_FragmentOffset & 0xDFFF;
    }
}

void setOffset(short Offset, short* X_DF_MF_FragmentOffset) {
    *X_DF_MF_FragmentOffset &= 0xE000;
    Offset &= 0x1FFF;
    *X_DF_MF_FragmentOffset |= Offset;
}
...
int main(int argc, char* argv[])
{
    ...
    unsigned short X_DF_MF_FragmentOffset = 0x00;
    unsigned char TimeToLive = 3;
    unsigned char Protocol = 1;
    short HeaderChecksum = 0x00;
    unsigned int SourceAddress = 1234;
    unsigned int DestinationAddress = 5678;
    char Packet[65536];
    ...
    while (1) {
        ...
        scanf("%s", datagram);
        unsigned int datagram_len = strlen(datagram);
...
/*-------------------------------------Framing------------------------------------------------*/
            len = framing(dst_mac_address, src_mac_address, 0xdd86, Packet, TotalLength, frame);
/*-------------------------------------Framing------------------------------------------------*/
            for (int i = 0; i < len; ++i) {
                frame[i] += 2;
            }
            frame[len] = '\0';
            sendto(sclient, frame, len, 0, (struct sockaddr *) &sin, len);
        }
        Identification += 1;
        int ret;
        ret = recvfrom(sclient, recvData, 255, 0, (struct sockaddr *) &sin, &len);
        if (ret > 0) {
            recvData[ret] = 0x00;
        ...
        // at the time of sending, so socket is directly used for receiving
    }
    closesocket(sclient);
    WSACleanup();
    return 0;
